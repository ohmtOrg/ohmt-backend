...
export const verifyRequestLineManager = async (req, res, next) => {
  try {
    ...
    const {
      dataValues: {
        User: { dataValues: requester },
        ...request
      },
    } = data;
    const isManager = loggedInUserId === data.User.lineManager;
    if (!isManager)
      return response(res, 401, 'error', { message: messages.unauthorized });
    req.request = request;
    ...
    if (requestId) {
      ...
      if (!request) {
        return response(res, 404, 'error', {
          message: messages.requestNotFoundId,
        });
      }
...
export const canAccessRequest = async (req, res, next) => {
  try {
    ...
    const { lineManager: requestUserLineManager } = await getById(
      User,
      userId,
      {
...
export const verifyEditRequestAuthorization = async (req, res, next) => {
  try {
    const {
      params: { requestId },
      decoded: { id: loggedInUserId },
    } = req;
    const [, requestType] = req.url.match(/\/([a-z]+)\/edit\/*/);
    let requestInfo, subrequestInfo;
    switch (requestType) {
      case 'requests':
        requestInfo = await getById(Request, requestId, {
          attributes: ['userId', 'approvalStatus'],
        });
        break;
      case 'subrequests':
        subrequestInfo = await getById(Subrequest, requestId, {
          attributes: ['requestId'],
        });
        requestInfo = await getById(Request, subrequestInfo.requestId, {
          attributes: ['userId', 'approvalStatus'],
        });
        break;
      default:
