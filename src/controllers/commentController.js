...
const { create, update, getAll, getByOptions, getById } = DbServices;
 ...
 */
const createComment = async ({ params, body, decoded, payload }, res) => {
  try {
    ...
    const receiverId =
      roleId === roles.REQUESTER ? user.lineManager : request.userId;
  ...
  try {
    ...
    if (checkComment)
      return response(res, 409, 'error', {
        message: messages.commentAlreadyDeleted,
      });
    if (!verifyOwner)
      return response(res, 409, 'error', {
        message: messages.commentOwnerNotFound,
      });
