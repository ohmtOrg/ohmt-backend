...
export const getAllNotifications = (req, res) =>
  handleMethodById(req, res, findByReceiver);
...
export const handleOptInEmail = (req, res) => {
  return handleMethodById(
    req,
    res,
    optInEmail,
    messages.optinEmailNotification
  );
};
...
export const handleOptOutEmail = (req, res) => {
  return handleMethodById(
    ...
    res,
    optOutEmail,
    messages.optoutEmailNotification
  );
};
...
export const handleMarkAsRead = async (req, res) => {
  try {
    ...
    if (notification.receiver !== userId)
      return response(res, 403, 'error', { message: messages.forbidden });
...
export const handleMarkAllAsRead = async (req, res) => {
  try {
    const { id } = req.decoded;
    const update = await markAllAsRead(id, {
      isRead: true,
      readDate: Date.now(),
    });
    response(res, 200, 'success', update);
