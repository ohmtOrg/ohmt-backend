...
const { create, getAll, bulkCreate, update } = DbServices;
...
const createTrip = async (
  ...
  if (!subRequest) {
    ...
    const subRequestedTrips = await bulkCreate(
      Subrequest,
      subRequestWithRequestId
    );
  ...
  try {
    ...
    if (!sender.lineManager)
      return response(res, 403, 'error', {
        message: 'You have not been assigned to a line manager',
      });
    const {
      type,
      originCity,
...
const getUserRequest = async (req, res) => {
  try {
    const {
      decoded: { id, roleId },
      query: { page, perPage, userId, approvalStatus },
    } = req;
    ...
    if (rows.length === 0)
      return response(res, 200, 'success', { message: noRequests });
...
const updateApprovalStatus = async (req, res) => {
  try {
    ...
    await createNotification({
      ...
      type:
        action === 'accept'
          ? notificationTypes.APPROVED_REQUEST
          : notificationTypes.REJECTED_REQUEST,
      ref: requestId,
    });
...
const updateTripRequest = async (req, res) => {
  try {
    ...
    await createNotification({
      sender,
      receiver,
      type: notificationTypes.EDITED_REQUEST,
      ref: requestId,
    });
