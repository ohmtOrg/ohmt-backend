...
import {
  createRating,
  getAverageRatingByAccommodation,
...
const { create, getById, getOrCreate, getAllRecord } = DbServices;
...
const bookAccommodation = async (req, res) => {
  try {
    ...
    if (!request)
      return response(res, 404, 'error', { message: messages.noResult });
    const accommodation = await getById(Accommodation, accommodationId, {});
    if (!accommodation)
      return response(res, 404, 'error', {
        message: messages.notExistAccommodation,
      });
    const { roomType } = accommodation;
    const inValidRoomArray = typeOfRoom.filter(
      roomName => !roomType.includes(roomName)
    );
    if (inValidRoomArray.length)
      return response(res, 400, 'error', {
        message: messages.invalidRoom(inValidRoomArray.join()),
      });
...
const accomodationFeedback = async (req, res) => {
  try {
    ...
    return response(res, 200, 'success', {
      message: accommodationFeedbackPosted,
    });
  } catch (error) {
  ...
  try {
    ...
    accommodations = await Promise.all(
      accommodations.map(async accommodation => {
        accommodation.dataValues.rating = await getAverageRatingByAccommodation(
          accommodation
        );
